...
#define SHORTEST_DB_URL_LEN (sizeof(SHORTEST_DB_URL) - 1)
 ...
 err:
	if (id->scheme) shm_free(id->scheme);
	if (id->username) shm_free(id->username);
	if (id->password) shm_free(id->password);
	if (id->host) shm_free(id->host);
	if (id->database) shm_free(id->database);
	if (prev_token) shm_free(prev_token);
	return -1;
}
...
struct db_id* new_db_id(const str* url)
{
	...
	ptr = (struct db_id*)shm_malloc(sizeof(*ptr));
	if (!ptr) {
