 ...
 */
int db_free_row_vals(db_row_t* _r)
{
	int	col;
	db_val_t* _val;
		...
		switch (VAL_TYPE(_val)) {
			case DB_STRING:
				if ( (!VAL_NULL(_val)) && VAL_FREE(_val)) {
					LM_DBG("free VAL_STRING[%d] '%s' at %p. free = %d\n", col,
							(char *)VAL_STRING(_val),
							(char *)VAL_STRING(_val),VAL_FREE(_val));
			...
			case DB_BLOB:
				if ( (!VAL_NULL(_val)) && VAL_FREE(_val)) {
					LM_DBG("free VAL_BLOB[%d] at %p. free = %d\n", col, VAL_BLOB(_val).s,VAL_FREE(_val));
