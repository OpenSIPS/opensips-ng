 ...
 * History:
 * ---------
 *  2010-03-28  adapted to 2.0 (bogdan)
 */
...
void free_mi_node(struct mi_node *parent)
{
	...
	if (parent->flags & MI_FREE_VALUE)
		shm_free(parent->value.s);
	if (parent->flags & MI_FREE_NAME)
		shm_free(parent->name.s);
	shm_free(parent);
}
							...
							char *name, int name_len, char *fmt_val, ...)
{
	...
	ret =  add_mi_node_sibling( brother, flags|MI_DUP_VALUE,
		name, name_len, p, len);
							...
							char *name, int name_len, char *fmt_val, ...)
{
	...
	ret = add_mi_node_child( parent, flags|MI_FREE_VALUE,
		name, name_len, p, len);
