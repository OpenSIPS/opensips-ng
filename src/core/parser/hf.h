/* 
 * $Id: hf.h 6344 2009-11-18 10:47:35Z andreidragus $ 
 *
...
struct hdr_field {
	...
	short len;              /**< length from hdr start until EoHF (incl.CRLF) */
	short body_buff_size;	/**< body buffer size, if explicitly allocated */
	short flags;			/**< flags for duplicating / freeing name and body */
	void* parsed;           /**< Parsed data structures */
	struct hdr_field* next; /**< Next header field in the list */
	struct hdr_field* prev;
	struct hdr_field* next_sibling; /**< Next header of same type */
	struct hdr_field* prev_sibling;
};
...
#define HDR_DUP_NAME			(1<<0)
#define HDR_DUP_BODY			(1<<1)
#define HDR_FREE_NAME			(1<<2)
#define HDR_FREE_BODY			(1<<3)
#define HDR_NOT_ORIG_MSG		(1<<4)
