/* 
 * $Id: dbase.c 6397 2009-12-10 16:14:23Z bogdan_iancu $ 
 *
...
static void reset_all_statements(  db_con_t* conn)
{
	//TODO 
 ...
 */
static int db_mysql_submit_query(  db_con_t* _h,   str* _s)
{
	int  code;
...
//	 LM_DBG("submit_query(): %.*s\n", _s->len, _s->s);
	...
	run_mysql_cmd( _h,
		mysql_send_query(CON_CONNECTION(_h),_s->s,_s->len),
		code, 1);
	if (code==0) return 0;
...
static int has_stmt_ctx( db_con_t* conn, struct my_stmt_ctx **ctx_p)
{
	struct my_stmt_ctx *ctx;
...
static struct my_stmt_ctx * get_new_stmt_ctx(  db_con_t* conn,
														  str *query)
{
	struct my_stmt_ctx *ctx;
	int code;
	...
	/* new one */
	ctx = (struct my_stmt_ctx*)shm_malloc
		( sizeof(struct my_stmt_ctx) + CON_TABLE(conn)->len + query->len);
	if (ctx==NULL) {
		LM_ERR("no more shm mem for statement context\n");
		return NULL;
	}
	memset( ctx, 0, 
		sizeof(struct my_stmt_ctx) + CON_TABLE(conn)->len + query->len);
	ctx->table.s = (char*)(ctx) + sizeof(struct my_stmt_ctx);
...
static struct prep_stmt* alloc_new_prepared_stmt(  db_val_t* v, int n,
			  db_val_t* uv, int un)
{
	struct prep_stmt *pq_ptr;
	MYSQL_TIME *mt;
	...
	pq_ptr = (struct prep_stmt*)shm_malloc( len );
	if (pq_ptr==NULL) {
		LM_ERR("no more shm mem for a new prepared statement\n");
		return NULL;
	}
	memset( pq_ptr, 0, len);
...
static int re_init_statement(  db_con_t* conn, struct prep_stmt *pq_ptr,
													struct my_stmt_ctx *ctx)
{
 ...
 **
 */
static int db_mysql_do_prepared_query(  db_con_t* conn,   str *query,
	  db_val_t* v, int n,   db_val_t* uv, int un)
{
	int i, code, cols;
	struct prep_stmt *pq_ptr;
	struct my_stmt_ctx *ctx;
...
void * db_mysql_init( db_id_t* id)
{
	...
	ptr = (struct my_con*)shm_malloc(sizeof(*ptr));
	if (!ptr) {
	...
	ptr->con = (MYSQL*)shm_malloc(sizeof(MYSQL));
	if (!ptr->con) {
		LM_ERR("no private memory left\n");
		goto err;
	}
...
err:
	if (ptr && ptr->con) shm_free(ptr->con);
	if (ptr) shm_free(ptr);
	return 0;
}
...
{
	struct my_con *_c;
	_c = (struct my_con *)_h->tail;

	LM_ERR("calling db_mysql_close \n");
	if (_c->ps_list)
		db_mysql_free_stmt_list(_c->ps_list);
	if (_c->res)
		mysql_free_result(_c->res);
	if (_c->id)
		free_db_id(_c->id);
	if (_c->con)
	{
...
static int db_mysql_get_result_info(  db_con_t* _h, db_res_t** _r)
{
	if ((!_h) || (!_r)) {
		LM_ERR("invalid parameter value\n");
		return -1;
	...
	*_r = db_new_result();
	...
	(*_r)->data = CON_RESULT(_h);

	if (db_mysql_fill_result(_h, *_r) < 0) {
		LM_ERR("error while converting result\n");
		shm_free(*_r);
		*_r = 0;
		/* all mem on opensips API side is already freed by
		 * db_mysql_fre_result in case of error, but we also need
...
{
	LM_DBG("freeing MYSQL specific data\n");
	       ...
	       db_key_t _o, db_res_t** _r)
{
	...
	if (CON_HAS_PS(_h)) {
		...
		if (query_holder.s)
			pkg_free(query_holder.s);
		if (ret!=0) return ret;
		ret = db_mysql_get_result_info(_h, _r);
		return ret;
	}
	return db_do_query(_h, _k, _op, _v, _c, _n, _nc, _o, NULL,
		db_mysql_val2str, db_mysql_submit_query, db_mysql_get_result_info,NULL);
}
...
{
	return db_do_raw_query(_h, _s, NULL, db_mysql_submit_query,
	db_mysql_get_result_info);
}
 ...
 * \return zero on success, negative value on failure
 */
int db_mysql_delete(  db_con_t* _h,   db_key_t* _k,   db_op_t* _o,
	  db_val_t* _v,   int _n)
{
	int ret;
	str query_holder = {0,0};
	...
	if (CON_HAS_PS(_h)) {
		if (CON_HAS_UNINIT_PS(_h)||!has_stmt_ctx(_h,&(CON_MYSQL_PS(_h)->ctx))){
			ret = db_do_update(_h, _k, _o, _v, _uk, _uv, _n, _un,
				db_mysql_val2str, NULL,&query_holder);
			if (ret!=0) 
			{
				if (query_holder.s)
					pkg_free(query_holder.s);
...
/** stores the next available row from the specified connection 
 * into the received db_row_t parameter
 *
 * return zero on succes, negative value on failure
 */
int db_mysql_fetch_next_row(db_res_t *_res,db_row_t *_r)
{
	db_con_t *_h = _res->it->connection;
	...
	if (db_mysql_convert_row(_h,_res,_r) < 0) 
	{
...
{
	return CON_CONNECTION(h)->net.fd;
}

int db_mysql_resume(db_con_t *h,db_res_t **r)
{
	if (mysql_read_query_result(CON_CONNECTION(h)) == 0)
	{
		if (r)
			db_mysql_get_result_info(h,r);
		return 0;
	}
