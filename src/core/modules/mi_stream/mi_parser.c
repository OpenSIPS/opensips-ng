 ...
 *          1 = end of stream
 */
static inline int mi_parse_node(fd_wrapper *wrap, str *buf,
		str *name, str *value)
{
	char *p, *pmax;
	char *start;
	...
	/* read one line */
	do {
		if (mi_get_next_line( &(buf->s), wrap->current_comm_len -
					wrap->mi_buf_pos+1, wrap->buffer+wrap->mi_buf_pos,
					&line_len)<0) {
			LM_ERR("failed proper parsing of MI command\n");
			return -1;
		}
	...
	do {
		...
		/*read one more line */
		if (mi_get_next_line( &(buf->s), wrap->current_comm_len -
					wrap->mi_buf_pos, wrap->buffer+ wrap->mi_buf_pos,
					&line_len)<0) {
			LM_ERR("failed proper parsing of MI command !\n");
			return -1;
		}
...
struct mi_root * mi_parse_tree(fd_wrapper *wrap) {
	...
	while ( (ret=mi_parse_node(wrap, &buf, &name, &value))>=0 ) {
		if (ret==1)
			return root;
		...
		if(!add_mi_node_child(node,MI_DUP_NAME | MI_DUP_VALUE,
					name.s,name.len,value.s,value.len)){
